...
| `secrets: inherit` + `with:` inputs (`pipeline_ref` everywhere; `max_wip` on all three promotion paths; `intake_hold` on the groomer stub — the reconcile stub's three Intake inputs are retired and read by nothing since DRE-4141) | `secrets:`/`inputs:` declarations |
...
how the CEO reads that proof. ONE **card** is where it happens, and it is the
epic's last child:
...
There is no second closing card. The CEO decided on 2026-09-12 that there are
...
and its body carries the closing line verbatim: `The CEO reads this record and
closes this card; there is no demo sitting.` The verdicts that MAY confirm an
epic are derived from `config/routing-verdicts.json` (the ones whose
accountable actor is a human, today `WORKBENCH` and `OPERATOR`), so the rule
...
epic missing the card back to `Planning` with the reason named — on the
