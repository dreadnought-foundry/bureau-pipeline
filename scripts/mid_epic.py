...
def _add(linear_ops, epic, because, title, body, labels, verdict, why) -> str:
    ...
    routing verdict, THEN the epic's artifact is updated, THEN the mid-epic
    verdict is posted. A crash between any two leaves the safe half: a card with
    no mid-epic verdict cannot promote (promotion_refusal), and a card the
    artifact never recorded is surfaced on the next sweep. The reverse order
    would leave a promotable card the plan never mentioned — the exact silent
    accretion this route exists to replace. The routing stamp sits FIRST of the
    three because the lane guard judges the create itself and reads that comment
    (DRE-3342): every write after it can be re-derived, and that one cannot be
    made early enough.
    """
    flags: list[str] = []
