...
def gh_read(*args: str) -> str:
    ...
    for attempt in range(1, GH_READ_ATTEMPTS + 1):
        ...
        if not retryable:
            ...
            # rate-limit refusal past the sweep's budget is still the fault
            # the seam handles (DRE-4214).
            fault = (
                ReconcileRateLimited if _is_rate_limit_refusal(stderr)
                else ReconcileReadError
            )
            raise fault(
