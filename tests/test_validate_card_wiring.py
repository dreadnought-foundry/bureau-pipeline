...
class PlanWiringTest(unittest.TestCase):
    ...
    def test_children_validated_before_green_light(self):
        # DRE-1715: the planner's created children are validated through the same
        # validate_card core BEFORE the epic moves to Green Light / completes.
        order = [s.lower() for s in steps_order(self.WF)]
        validate = next(
        ...
        review = next(i for i, s in enumerate(order) if "green light" in s)
        self.assertLess(validate, review, "children must be validated before Green Light")
...
class PlanModelFallbackWiringTest(unittest.TestCase):
    ...
    The planner walks the `judgement` ladder (claude-fable-5-1, then Opus, then
    Sonnet — DRE-3015), so a re-dispatched plan run must ride the next model
    ...
    wiring replaced — a DIFFERENT id from the one the ladder now names, still
    excluded from every ladder, and still a model no role may select at all.
    """
