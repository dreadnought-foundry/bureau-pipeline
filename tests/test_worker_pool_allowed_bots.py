...
DRE-2037 extends the invariant: every bot identity that can LEGITIMATELY
trigger a workflow must be in that workflow's allowed_bots. The DRE-1924
merge gate updates a behind branch as agent-bureau-qa-bot, whose push fires
`pull_request: synchronize` on qa-review and verify — so those two must
...
class PoolCoversEveryWorkerAllowlistTest(unittest.TestCase):
    ...
    def test_worker_allowlists_exist(self):
        ...
        self.assertGreaterEqual(
            ...
            f"verify.yml (x2), agent-task.yml, plan.yml (x7); found {worker_sites}",
        )
    ...
    def test_expected_files_each_carry_a_pooled_worker_allowlist(self):
        ...
        # the wave route (DRE-2845), and the card classifier's one bounded
        # call (DRE-3074 — it moved off the raw Messages API onto the same
        # Claude Code transport every other model call here uses).
        expected = {
            "qa-review.yml": 2,
            "verify.yml": 2,
            "agent-task.yml": 1,
