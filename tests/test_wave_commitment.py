...
class TestApprovingTheWaveGreenLightsNothing:
    ...
    def test_committing_a_wave_moves_the_first_epic_and_dispatches_nothing_itself(self):
        """The lane move IS the trigger: the relay dispatches `agent-plan` for
        every card entering the lane that owes a plan artifact (DRE-1913,
        DRE-3030). This path used to ask for the run as well, and every wave's
        first epic got two Agent Plan runs, the second starting a hosted runner
        to learn it was the duplicate (DRE-3659). So the commit moves the epic
        and shells out to nothing — one dispatcher per transition."""
    ...
    def test_the_arrival_note_says_the_lane_entry_starts_the_run(self):
        """A receipt nobody can check is the DRE-1254 false-receipt class: the
        note must not claim a run this module did not start. It says what does
        start it, and what asks if nothing has."""
        ops = _FakeOps(description=wc.render_ledger(ledger_of()), green_lit_at=APPROVED)
        wc.commit(ops, WAVE)
        moved = [ident for ident, _ in ops.advanced_to]
        said = "\n".join(ops.comments_on(moved[0]))
        assert "has been started" not in said
        assert f"`{wc.turn_lane()}`" in said
        assert "stalled-planning alarm" in said
