...
**A re-armed run walks from the head it wakes to (DRE-3791).** A dispatched run's `github.sha` is fixed at dispatch, and the plan's checkout has no `ref:` — so a run that slept would walk from the commit it was dispatched at. The plan is handed the run's `not_before`; a run that carries one fetches the default branch's tip from `origin`, moves its checkout there, and walks from it, so a merge that is green when the train fires is in the release and one still checking is stepped past as ever. The run's line names the head it walked from and the one it was dispatched at. A tip that cannot be re-read is a `::warning::` naming the dispatch-time commit the run walks from instead. Every run that was not re-armed walks from the head it was handed.
