...
| **exit** | A classification exists: one-off, epic, or wave. There are exactly TWO ways out, and both are an approval the CEO gives. For a card nobody filed under an epic the way out is the groomer — it proposes a batch, the CEO approves it in Green Light, and the approved batch drains to Planning (groomer.py drain). For a card whose PARENT EPIC is being planned the way out is adoption (DRE-4668, on the CEO's signed answer of 2026-09-23): the planning exit writes the child's routing verdict and moves it to Backlog, where it waits — the epic's own green light is the approval for the cards filed under it, and nothing builds until the CEO moves that epic to In Progress. A child the verdict sends back to Planning is not adopted and stays here. AGE IS NOT AN EXIT: no card is moved out of this lane for being old, this lane carries no stall window, and there is no window of any length in its place (DRE-4141, on the CEO's signed answer of 2026-09-17). The sweep reports how many cards are waiting and how old the oldest is, and moves none of them. | Phase 5 — promised |
| **writers** | Anything that creates a card, and the retroactive pass that moves the legacy Backlog in. ONE of them also moves a card onward, and only with an approval behind it: plan.yml adopts a planned epic's own children into Backlog (DRE-4668). The groomer's drain — the other way out — writes Planning rather than this lane, so it is named as a writer there.  
...
| Clause | What it requires | Enforcement |
| --- | --- | --- |
| **entrance** | A plan is waiting on the CEO, an agent has escalated a question only the CEO can answer, the planner has escalated a card whose reasoning is itself the deliverable and stated why (DRE-2848), the post-approval critic sent an approved plan back with a stated reason (DRE-2721), a card has aged out of Intake and the sweep moved it here (DRE-2687), or a card has stood still in Planning past that lane's window and the sweep moved it here (DRE-4124 — the same remedy as the Intake age-out, one lane later: it used to be labelled for a human and left in Planning, where every other mechanism skips a held card and nobody was ever asked).  
...
| **exit** | The verdict's destination is reached, and the sweep is what reaches it (DRE-3385). It promotes a FLEET card to Todo once the dependency gate clears, the WIP cap has room and — for a child of an epic — the second critic has passed the plan since it was approved (DRE-3059). The sweep promotes a WORKBENCH or OPERATOR card to Todo on the same gates too, stamping the marks its verdict declares — `hand-built`, plus `no-code` for OPERATOR — before the move, so the card never sits in Todo unmarked; nothing is dispatched for it, it takes no slot under the WIP cap, and the receipt says whose turn it is. A PARKED card does not leave, and is never reported as stalled; a NEEDS WORK card goes back to Planning, which is the planner's move and not the sweep's. A card carrying no verdict at all is not promoted, whether or not it has a parent epic: the epic's approval says the PLAN was approved, never who builds this piece of it. A verdict approves ONE trip through Planning (DRE-4962): a card that has entered Intake, Green Light, Triage, Canceled, Duplicate or Done since its newest verdict is not promoted — the sweep refuses it `stale-verdict`, says once on the card when the verdict was written, which of those lanes it passed through since and that it needs Planning again to be built, and leaves it here. A later exit from Planning, or for a child its epic's green light, is a fresh approval, because the planning exit does not re-stamp a verdict the card already carries. The check reads the card's lane history, so it is the last gate, asked only of a card every other gate has passed; a history Linear cannot return holds the card that sweep and says nothing on it.  
_Waiting on: the sweep carries every Todo-bound verdict out of this lane and refuses the rest (DRE-2724, DRE-3385); asserting the transitions needs the history Phase 5 records._ | Phase 5 — promised |
| **writers** | The planner and mid-epic discovery create here; the sweep and the dead-run cap park here; and the build run parks the card it could not build — a stated blocker, or a run that kept dying past the cap (DRE-2859). A person also lands a card here by sending it back from Green Light — the rejected answer that lane's exit names — or by moving it here by hand; that move approves nothing, and the `stale-verdict` refusal above is what keeps the sweep from building it (DRE-4962).  
...
_Waiting on: the sweep promotes a FLEET card and a marked hand-built one (DRE-2724, DRE-3385); asserting occupancy needs the transition history Phase 5 records._ | Phase 5 — promised |
